...
class Media extends Component {
  ...
  async componentDidMount(){
      ...
      if(response.ok){
          ...
          var photoData = [];
          // try {
              photos.some((element,index) => {
                  if(index === 16){
                      // throw dataOverflowException;
                      console.log(index);
                      return true;
                  }
                  photoData.push({id: element.id,
                      name: element.title,
                      image: element.url});
              });
          // } catch (dataOverflowException) {
          //     console.log('Data overflow !!! Exit.');
          // }
          this.setState({data: photoData, isLoading:false});
      }else{
          console.log('Error while fetching data from /photos');
